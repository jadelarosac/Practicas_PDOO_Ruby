...
class TextMainView
  ...
  def showTrasformationMessage(transform)
    if transform == Deepspace::Transformation::GETEFFICIENT
      puts "La estación espacial ha cambiado su eficiencia"
    elsif transform == Deepspace::Transformation::SPACECITY
      puts "Gracias a la diplomacia, la estación espacial se ha convertido en la base de una nueva ciudad" 
    end
  end
